import React from "react";

interface TelegramIconProps {
  color?: string;
  size?: number;
  className?: string;
}

const TelegramIcon: React.FC<TelegramIconProps> = ({
  color = "black",
  size = 24,
  className = "",
}) => {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
    >
      <path
        d="M20.71 3.65451C20.71 3.65451 22.6525 2.89701 22.49 4.73651C22.4365 5.49401 21.951 8.14551 21.573 11.013L20.278 19.508C20.278 19.508 20.17 20.7525 19.1985 20.969C18.2275 21.185 16.7705 20.2115 16.5005 19.995C16.2845 19.8325 12.4535 17.3975 11.1045 16.2075C10.7265 15.8825 10.2945 15.2335 11.1585 14.476L16.8245 9.06501C17.472 8.41501 18.1195 6.90001 15.4215 8.74001L7.86649 13.88C7.86649 13.88 7.00299 14.4215 5.38449 13.9345L1.87649 12.852C1.87649 12.852 0.581494 12.0405 2.79399 11.229C8.19049 8.68601 14.828 6.08901 20.7095 3.65401"
        fill={color}
      />
    </svg>
  );
};

export default TelegramIcon;
